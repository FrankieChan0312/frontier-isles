...
it('caps Room allocation at 64 and permits a new Room after an existing Room closes', () => {
  const service = new InMemoryRoomService(); cleanup.push(() => service.dispose())
  const first = successData(service.createRoom('First'))
  for (let index = 1; index < MAX_ROOMS; index += 1) successData(service.createRoom(`Host ${index}`))
  expect(service.createRoom('Over capacity')).toMatchObject({ ok: false, error: { code: 'SERVER_BUSY' } })
  expect(service.roomCount).toBe(64)
  successData(service.leaveRoom(first.credential.sessionId))
  successData(service.createRoom('New Host'))
  expect(service.roomCount).toBe(64)
})
it('quarantines oversized private rows without decoding them and preserves an unrelated valid Room', () => {
  ...
  const room = persistentRoom(initial); cleanup.push(() => room.service.dispose())
  const before = requireValue(initial.load()[0]); initial.close()
  ...
  const repository = new SqliteMultiplayerRepository(directory.database); cleanup.push(() => repository.close())
  expect(repository.load()).toEqual([before]); repository.close()
...
it('refuses a database beyond the alpha Room cap without deleting or resetting any records', () => {
  const directory = temporaryPersistenceDirectory(); cleanup.push(directory.remove)
  const initial = new SqliteMultiplayerRepository(directory.database); initial.close()
  const writer = new DatabaseSync(directory.database)
  for (let index = 0; index <= MAX_ROOMS; index += 1) writer.prepare('INSERT INTO rooms VALUES (?,?,?)').run(`R${index}`, '{}', '0'.repeat(64))
  writer.close()
  const repository = new SqliteMultiplayerRepository(directory.database); cleanup.push(() => repository.close())
  expect(() => repository.load()).toThrow('PERSISTENCE_OPEN_FAILED'); repository.close()
  const inspect = new DatabaseSync(directory.database)
...
it('releases database ownership after a checkpoint failure while preserving previously committed records', () => {
  const directory = temporaryPersistenceDirectory(); cleanup.push(directory.remove)
  const repository = new SqliteMultiplayerRepository(directory.database)
  const room = persistentRoom(repository); cleanup.push(() => room.service.dispose())
  const before = repository.load()
  vi.spyOn(repository, 'flush').mockImplementation(() => { throw new Error('PRIVATE_CHECKPOINT_FAILURE') })
  expect(() => repository.close()).toThrow('PERSISTENCE_WRITE_FAILED')
  const reopened = new SqliteMultiplayerRepository(directory.database); cleanup.push(() => reopened.close())
  expect(reopened.load()).toEqual(before)
})
