...
export function defineRecoveryContract(openStorage: () => { readonly open: () => MultiplayerRepository; readonly remove: () => void }): void {
const cleanup: (() => void)[] = []
...
function fixture(dependencies: GameSessionDependencies = { createState: actionFixture }, humans = 4) {
  ...
  const repository = directory.open()
  ...
  const restart = (runtime = new FakeLifecycleRuntime(), nextDependencies: GameSessionDependencies = {}) => {
    ...
    repository.close()
    const restoredRepository = directory.open()
