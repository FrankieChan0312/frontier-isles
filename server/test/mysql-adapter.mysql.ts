...
async function recordFixture(start = true) {
  ...
  if (start) await room.start()
  return { room, record: requireValue((await repository.load())[0]) }
}
async function store(options: Parameters<typeof MysqlStore.open>[1] = {}) {
  ...
  cleanup.push(async () => (await result.close()))
  return result
}
describe('real MySQL aggregate storage', () => {
  it('boots empty, creates, updates, preserves canonical bytes and token privacy, deletes and closes', async () => {
    const diagnostics: PersistenceDiagnostic[] = []
    const repository = (await MysqlMultiplayerRepository.open(mysqlTestConfig(), { onDiagnostic: (value) => diagnostics.push(value) }))
    cleanup.push(async () => (await repository.close()))
    expect((await repository.load())).toEqual([])
    const { room, record } = (await recordFixture())
    ...
    const stored = canonicalJson((await repository.load())[0])
    ...
    await repository.remove(record.roomCode)
    expect((await repository.load())).toEqual([])
    await repository.flush()
    await repository.close()
    await repository.close()
    await expect(repository.flush()).rejects.toThrow('PERSISTENCE_WRITE_FAILED')
  ...
  it('does not acknowledge or publish a failed database mutation and retries once after recovery', async () => {
    ...
    const before = canonicalJson((await repository.load()))
    ...
    const restored = (await MysqlMultiplayerRepository.open(mysqlTestConfig()))
    cleanup.push(async () => (await restored.close()))
    expect(canonicalJson((await restored.load())) === before).toBe(true)
    const service = (await restoredService(restored, new FakeLifecycleRuntime()))
    cleanup.push(() => service.dispose())
    await resumeEveryHuman(service, room.members)
  ...
  it('quarantines checksum, malformed, future, oversized and colliding-session records without private diagnostics', async () => {
    ...
    const { record } = (await recordFixture(false))
    const writer = await mysqlTestConnection()
    cleanup.push(() => writer.end())
