...
describe('InMemoryRoomService', () => {
  it('creates revision zero with one NORTH Host and private credentials', async () => {
    const service = (await InMemoryRoomService.open({ generators: deterministicGenerators() }))

    const data = successData((await service.createRoom('Ada')))
  ...
  it('joins Humans in canonical order and rejects normalized duplicate names', async () => {
    const service = (await InMemoryRoomService.open({ generators: deterministicGenerators() }))
    const creator = successData((await service.createRoom('Ada')))

    const joiner = successData((await service.joinRoom(creator.credential.roomCode, 'Grace')))
    ...
    const duplicate = (await service.joinRoom(creator.credential.roomCode, 'Grace'))
  ...
  it('rejects unknown Rooms and a fifth Human without mutating a full Room', async () => {
    const service = (await InMemoryRoomService.open({ generators: deterministicGenerators() }))
    expect((await service.joinRoom(roomCodeSchema.parse('ZZZ999'), 'Grace'))).toMatchObject({
      ok: false,
      error: { code: 'ROOM_NOT_FOUND' },
    })

    const creator = successData((await service.createRoom('Ada')))
    successData((await service.joinRoom(creator.credential.roomCode, 'Grace')))
    successData((await service.joinRoom(creator.credential.roomCode, 'Linus')))
    const fullSnapshot = successData((await service.joinRoom(creator.credential.roomCode, 'Margaret'))).snapshot

    expect((await service.joinRoom(creator.credential.roomCode, 'Edsger'))).toMatchObject({
      ok: false,
  ...
  it('increments Ready changes once and treats a repeated value as an idempotent no-op', async () => {
    const service = (await InMemoryRoomService.open({ generators: deterministicGenerators() }))
    const creator = successData((await service.createRoom('Ada')))

    const changed = successData((await service.setReady(
    ...
    const noOp = successData((await service.setReady(
      creator.credential.sessionId,
    ...
    const stale = (await service.setReady(
      creator.credential.sessionId,
  ...
  it('lets only the Host assign AI and never overwrites a Human', async () => {
    const service = (await InMemoryRoomService.open({ generators: deterministicGenerators() }))
    const creator = successData((await service.createRoom('Ada')))
    const joiner = successData((await service.joinRoom(creator.credential.roomCode, 'Grace')))

    const nonHost = (await service.setAiSeat(
    ...
    const humanSeat = (await service.setAiSeat(
      creator.credential.sessionId,
      joiner.snapshot.revision,
      'EAST',
      'MERCHANT',
    ...
    const assigned = successData((await service.setAiSeat(
      creator.credential.sessionId,
      joiner.snapshot.revision,
      'SOUTH',
      'BUILDER',
    ...
    const noOp = successData((await service.setAiSeat(
      creator.credential.sessionId,
  ...
  it('derives future start readiness but always rejects room:start in Goal A', async () => {
    const service = (await InMemoryRoomService.open({ generators: deterministicGenerators() }))
    const creator = successData((await service.createRoom('Ada')))
    const joiner = successData((await service.joinRoom(creator.credential.roomCode, 'Grace')))
    const south = successData((await service.setAiSeat(
      creator.credential.sessionId,
      joiner.snapshot.revision,
      'SOUTH',
      'MERCHANT',
    )))
    const west = successData((await service.setAiSeat(
      creator.credential.sessionId,
      south.snapshot.revision,
      'WEST',
      'SENTINEL',
    )))
    const creatorReady = successData((await service.setReady(
      creator.credential.sessionId,
      west.snapshot.revision,
      true,
    )))
    const allReady = successData((await service.setReady(
      joiner.credential.sessionId,
    ...
    expect((await service.requestStart(
      creator.credential.sessionId,
      allReady.snapshot.revision,
    ))).toMatchObject({ ok: true, data: { snapshot: { lifecycleStatus: 'ACTIVE' } } })
  })

  it('transfers Host on leave and closes the Room after the last Human leaves', async () => {
    const service = (await InMemoryRoomService.open({ generators: deterministicGenerators() }))
    const creator = successData((await service.createRoom('Ada')))
    const joiner = successData((await service.joinRoom(creator.credential.roomCode, 'Grace')))

    const hostLeave = successData((await service.leaveRoom(creator.credential.sessionId)))
    ...
    const finalLeave = successData((await service.leaveRoom(joiner.credential.sessionId)))
  ...
  it('bounds RoomCode collision retries and keeps rooms isolated', async () => {
    const generators = deterministicGenerators(
      ['ABC234', 'DEF567', 'ABC234', 'ABC234'],
      ['session_000000001', 'session_000000002'],
    )
    const service = (await InMemoryRoomService.open({ generators, maximumIdentifierAttempts: 2 }))
    const first = successData((await service.createRoom('Ada')))
    const second = successData((await service.createRoom('Grace')))
    expect(first.snapshot.roomCode).toBe('ABC234')
    expect(second.snapshot.roomCode).toBe('DEF567')

    const exhausted = (await service.createRoom('Linus'))
