...
async function fixture(humans = 4, dependencies: GameSessionDependencies = {}) {
  const runtime = new FakeLifecycleRuntime()
  let state: GameState | null = null
  const rooms = (await InMemoryRoomService.open({ runtime, reconnectGraceMs: 100, gameAbandonedTtlMs: 1_000,
  ...
  services.push(rooms)
  const members: RoomSessionData[] = [successData((await rooms.createRoom('North')))]
  ...
  for (const name of ['East', 'South', 'West'].slice(0, humans - 1)) members.push(successData((await rooms.joinRoom(code, name))))
  for (const seat of snapshot().seats) if (seat.occupancy === 'EMPTY') successData((await rooms.setAiSeat(first.credential.sessionId, snapshot().revision, seat.seatId, 'BUILDER')))
  for (const member of members) successData((await rooms.setReady(member.credential.sessionId, snapshot().revision, true)))
  successData((await rooms.requestStart(first.credential.sessionId, snapshot().revision)))
...
describe('active-game Human presence policy', () => {
  it('disposes idempotently before late socket disconnect callbacks without rebuilding closed presence', async () => {
    const room = (await fixture())
    room.rooms.dispose()
    room.rooms.dispose()
    expect((await room.rooms.markDisconnected(room.session(0)))).toMatchObject({ ok: false })
    expect((await room.rooms.createRoom('Late arrival'))).toMatchObject({ ok: false, error: { code: 'ROOM_CLOSED' } })
  ...
  it.each([0, 1])('pauses for disconnected Human %s, rejects every new Human mutation, and preserves exact state/RNG', async (index) => {
    ...
    successData((await room.rooms.markDisconnected(room.session(index))))
  ...
  it('resumes at deadline minus one with the same identity/view and expires exactly at the boundary', async () => {
    const room = (await fixture())
    const credential = requireValue(room.members[1]).credential
    const before = room.game.snapshot(credential.sessionId).view
    successData((await room.rooms.markDisconnected(credential.sessionId)))
    await room.runtime.advanceBy(99)
    const resumed = successData((await room.rooms.resumeSession(credential)))
    ...
    expect((await room.rooms.resumeSession(credential))).toMatchObject({ ok: false, error: { code: 'SESSION_INVALID' } })
  ...
  it.each(['SEVEN', 'KNIGHT', 'BUILD_TRADE'] as const)('retains pending private decisions and hands through %s disconnect/resume', async (scenario) => {
    const room = (await fixture(4, { createState: (config) => workflowFixture(scenario, config) }))
    ...
    successData((await room.rooms.resumeSession(requireValue(room.members[index]).credential)))
    expect(room.state()).toBe(before)
  ...
  it('returns an earlier exact cached outcome while paused without accepting a new mutation', async () => {
    ...
    successData((await room.rooms.markDisconnected(room.session(1))))
    const before = room.state()
  ...
  it('requires all disconnected Humans to recover before clearing the pause', async () => {
    const room = (await fixture())
    for (const index of [1, 2]) successData((await room.rooms.markDisconnected(room.session(index))))
    successData((await room.rooms.resumeSession(requireValue(room.members[1]).credential)))
    expect(room.game.lifecycleStatus).toBe('PAUSED_RECONNECTING')
    expect(room.game.presenceSnapshot.disconnectedSeats.map((seat) => seat.seatId)).toEqual(['SOUTH'])
    successData((await room.rooms.resumeSession(requireValue(room.members[2]).credential)))
    expect(room.game.lifecycleStatus).toBe('ACTIVE')
  })
  ...
  it('permits only the connected current Host after expiry, preserving the replaced position with an AI overlay', async () => {
    ...
    const room = (await fixture(4, { aiAgent: { chooseNextCommand: async (view, context) => {
      views.push(view)
    ...
    expect(room.game.lifecycleStatus).toBe('ACTIVE')
    expect((await room.rooms.resumeSession(requireValue(room.members[1]).credential))).toMatchObject({ ok: false, error: { code: 'SESSION_INVALID' } })
  ...
  it('transfers an expired Host to the first connected Human, including one who resumes while others are still in grace', async () => {
    const room = (await fixture())
    successData((await room.rooms.markDisconnected(room.session(0))))
    await room.runtime.advanceBy(10)
    for (const index of [1, 2, 3]) successData((await room.rooms.markDisconnected(room.session(index))))
    await room.runtime.advanceBy(90)
  ...
  it('selects EAST before other connected Humans when the Host expires', async () => {
    const room = (await fixture())
    successData((await room.rooms.markDisconnected(room.session(0))))
    await room.runtime.advanceBy(100)
  ...
  it.each([false, true])('does not apply a held AI choice spanning a pause (resume before choice returns: %s)', async (earlyResume) => {
    ...
    const agent = new PersonalityAiAgent()
    let choices = 0
    const room = (await fixture(2, { createState: (config, seed) => {
    ...
    if (earlyResume) successData((await room.rooms.resumeSession(requireValue(room.members[1]).credential)))
    release()
    await advance
    // Disconnect now cancels the held choice so its Room queue can persist presence.
    // Transport resume explicitly starts the next AI pipeline after its commit.
    if (earlyResume) await room.game.advanceAi()
    if (!earlyResume) {
      expect(room.state()).toBe(before)
      expect(choices).toBe(1)
      successData((await room.rooms.resumeSession(requireValue(room.members[1]).credential)))
  ...
  it('keeps an all-disconnected game within grace, then closes when no eligible Humans remain', async () => {
    const room = (await fixture())
    const before = room.state()
    for (const member of room.members) successData((await room.rooms.markDisconnected(member.credential.sessionId)))
    await room.runtime.advanceBy(99)
    expect(room.rooms.getSnapshot(room.code)).not.toBeNull()
    await room.runtime.advanceBy(1)
  ...
  it('allows Host closure only while replacement is required and clears every timer and credential', async () => {
    const room = (await fixture())
    const close = async (host = 0) => (await room.rooms.closeActiveGame(room.session(host), room.game.gameId, room.snapshot().revision))
    expect((await close())).toMatchObject({ ok: false, error: { code: 'REPLACEMENT_NOT_AVAILABLE' } })
    successData((await room.rooms.markDisconnected(room.session(1))))
    await room.runtime.advanceBy(100)
    expect((await close(2))).toMatchObject({ ok: false, error: { code: 'NOT_HOST' } })
    successData((await close()))
    expect(room.game.lifecycleStatus).toBe('CLOSED')
    expect(room.runtime.activeTaskCount).toBe(0)
    for (const member of room.members) expect(room.rooms.hasSession(member.credential.sessionId)).toBe(false)
  })

  it('expires an abandoned replacement decision without extending its deadline on snapshot reads', async () => {
    const room = (await fixture())
    successData((await room.rooms.markDisconnected(room.session(1))))
    await room.runtime.advanceBy(100)
    expect(room.snapshot().gamePresence?.abandonedDeadlineMs).toBe(1_100)
    await room.runtime.advanceBy(999)
    successData((await room.rooms.requestSnapshot(room.session(0))))
    await room.runtime.advanceBy(1)
    expect(room.rooms.getSnapshot(room.code)).toBeNull()
    expect(room.runtime.activeTaskCount).toBe(0)
  })
  ...
  it('retains a completed game for its configured interval and then cleans it up', async () => {
    const room = (await fixture(4, { createState: (config) => workflowFixture('VICTORY', config) }))
    const north = room.session(0)
